...
def main(X, y):
    ...
    plt.title("Error Rate vs Number of MODMA Features")
    plt.grid(True)
    plt.savefig('/home/zlh/gabor/results/modma/error_rate.png',bbox_inches='tight', dpi=300)
    plt.close()
...
# 数据
data = np.load("MODMA_features_labels.npz")
X = data['features']
arr2 = data['labels']
y = [1 if label =='MDD' else 0 for label in arr2]
...
# print("特征选择完成，已保存为 MODMA_features_labels_selected.npz")

#  未进行特征选择前的可视化
visualize_features(X, y,title="Original Features", save_path='/home/zlh/gabor/results/modma/original_features.png')

# 特征选择后的可视化
visualize_features(last, y, title="Selected Features", save_path='/home/zlh/gabor/results/modma/selected_features.png')

# file_path='emobase1.csv'
# df = pd.read_csv(file_path,header=None)
# rows_to_extract = best_feature_indices
# extracted_data = df.iloc[rows_to_extract]
# extracted_data.to_csv('/home/zlh/gabor/feature csv/selected_features.csv')
# print("特征选择完成，已保存为 selected_features.csv")